...
where we can specifiy Nolan'sparametrization p (in (0,1,2), default 1)
...
```julia
# All following distribution are using Nolan'sparametrization p (in (0,1,2), default 1)

Stablep(α;p)            # standard symmetric α-Stable distribution equivalent to Stablep(α, 0, 1, 0;p)
Stablep(α, β;p)         # standard α-Stable distribution with skewness parameter β equivalent to S(α, β, 1, 0;p)
Stablep(α, β, γ, δ;p)   # α-Stable distribution with skewness parameter β, scale γ and location δ

params(d)               # Get the parameters, i.e. (α, β, γ, δ, p)
...
"""
struct Stablep{T<:Real} <: ContinuousUnivariateDistribution
    α::T
    β::T
    γ::T
    δ::T
    ...
    σ1::T
    μ1::T
    function Stablep{T}(α, β, γ, δ; p, check_args::Bool=true) where {T}
        @check_args Stable (α, zero(T) < α <= 2one(T)) (β, -one(T) <= β <= one(T)) (γ, zero(T) < γ) ((α, β), α != 2 || β == 0 )
        (α1,β1,σ1,μ1)=_paramconvstabledists(p,1,α,β,γ,δ)
        new{T}(α, β, γ, δ, p, α1, β1, σ1, μ1)
    end
end
...
StablefromStablep(dp::Stablep)   = Stable(dp.α1, dp.β1, dp.σ1, dp.μ1)
StablepfromStable(d::Stable;p=1) = Stablep(_paramconvstabledists(1,p,params(d)...)...;p=p)

Stablep2(α, β, γ, δ) = Stablep(α, β, γ, δ;p=2)
Stablep2(α, β)       = Stablep2(α, β  , 1.0,0.0)
Stablep2(α,)         = Stablep2(α, 0.0, 1.0,0.0)

Stablep1(α, β, γ, δ) = Stablep(α, β, γ, δ;p=1)
Stablep1(α, β)       = Stablep1(α, β  , 1.0,0.0)
Stablep1(α,)         = Stablep1(α, 0.0, 1.0,0.0)

Stablep0(α, β, γ, δ) = Stablep(α, β, γ, δ;p=0)
Stablep0(α, β)       = Stablep0(α, β  , 1.0,0.0)
Stablep0(α,)         = Stablep0(α, 0.0, 1.0,0.0)
...
convert(::Type{Stablep{T}}, α::S, β::S, γ::S, μ::S;p::Int) where {T <: Real, S <: Real} = Stablep(T(α), T(β), T(γ), T(δ);p=p)
Base.convert(::Type{Stablep{T}}, d::Stablep) where {T<:Real} = Stablep{T}(T(d.α), T(d.β), T(d.γ), T(d.δ);p=p)
Base.convert(::Type{Stablep{T}}, d::Stablep{T}) where {T<:Real} = d

#### Parameters

shape(d::Stablep) = (d.α, d.β)
location(d::Stablep) = d.γ
scale(d::Stablep) = d.δ
params(d::Stablep) = (d.α, d.β, d.γ, d.δ)
partype(::Stablep{T}) where {T} = T

paramsp(d::Stablep) = (d.α, d.β, d.γ, d.δ, p)
parametrization(d::Stablep)= d.p
params1(d::Stablep) = (d.α, d.β, d.σ1, d.μ1)

#### Statistics

mean(d::Stablep{T}) where T = d.α > one(T) ? d.μ1 : T(NaN)
var(d::Stablep{T}) where T = d.α == 2one(T) ? 2d.σ1^2 : T(Inf)
skewness(d::Stablep{T}) where T = d.α == 2one(T) ? T(0.0) : T(NaN)
kurtosis(d::Stablep{T}) where T = d.α == 2one(T) ? T(0.0) : T(NaN)

#### Evaluation

# ok for parametrization 1, to be checked for parametrization 0 and 2
function cf(d::Stablep{T}, t::Real) where T
    ds=Stable(d.α1, d.β1, d.σ1, d.μ1)
    return cf(ds,x)
end

# ok for parametrization 1, to be checked for parametrization 0 and 2
function mgf(d::Stablep{T}, t::Real) where T
    ds=Stable(d.α1, d.β1, d.σ1, d.μ1)
    return mgf(ds,x)
end

function pdf(d::Stablep{T}, x::Real) where T
    ds=Stable(d.α1, d.β1, d.σ1, d.μ1)
    return pdf(ds,x)
end
...
function cdf(d::Stablep{T}, x::Real) where T
    ds=Stable(d.α1, d.β1, d.σ1, d.μ1)
    return cdf(ds,x)
end
...
function mode(d::Stablep{T}) where T
    ...
    if p == 2 ; return 0.0; end
    if p == 0; return γ*_mm(α,β)+δ; end
    if p != 1; error("StableDistributionsp : p=$p != 1"); end
    # if p == 1
    d=0.0
    if !(α ≈ 1.0)
        d=δ+β*γ*tanpi(α/2)
...
Base.:+(d::Stablep, a::Real) = Stablep(d.α, d.β, d.γ, d.δ + a; p=d.p)
...
function fit(::Type{<:Stablep}, x::AbstractArray{<:Real};p=1)
    ...
    (α, β, γ, δ) = _paramconvstabledists(1,p,params(dStablefitted)...)
